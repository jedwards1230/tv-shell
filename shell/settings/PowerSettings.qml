...
// Power settings page. Migrated onto SettingsPageBase (#5): the page-scaffold
// (padded content column, trailing spacer, bottom HintBar via hintText) lives in
// the base. The ConfirmDialog overlay and the non-visual IPC helpers go in the
// base's `overlay` slot so they don't take a content-column layout slot.
SettingsPageBase {
    id: root
    hintText: "A: Select  |  Use with caution"
    ...
    // === Page content (lands in the base's content column) ===
    SectionHeader {
        text: "Power Settings"
    }
    ...
    // Spacer between the settings group and the power-action cards. This is
    // intentional mid-content spacing (NOT the base's trailing spacer).
    Item {
    ...
    // === Overlay / non-flow slot: IPC helpers + the confirm dialog ===
    // Suspend routes through the input daemon's logind-over-zbus backbone
    // (Phase 3). Reboot/poweroff remain one-shot `systemctl` actions.
    overlay: [
        Process {
            id: powerOff
