...
> **The plan for actually building this — phases, open decisions, and the
> hardware measurements it is gated on — is
> [`V2_GAMEPAD_HANDOFF.md`](V2_GAMEPAD_HANDOFF.md).** Two questions that
> document carried as open are now closed by measurement on the reference
> deployment, and both are load-bearing here. **The core can drive the shell
> with a uinput keyboard** (M0, 2026-09-10): a synthesised key arrives exactly
> as a physical one does, with Moonlight running and suspended, and needs no
> seat tagging. The 2026-09-09 negative was an artefact of the probe key —
> gamescope silently drops `KEY_MENU`, which was the drawer's only binding
> (jedwards1230/tv-shell#489, fixed in #490 by moving the drawer to Tab), so any
> code the core emits must be verified to arrive rather than assumed. And **an
> idle presenter is enumerated by a running app** (M1, 2026-09-10): with the
> physical pad ungrabbed, Moonlight opened the virtual node within ~2 s and held
> it alongside the real one, so the app saw two controllers. That settles the
> grab question against the `gamepad` contract as originally written; see the
> amended bullet below.
...
The chain is HTPC HDMI → the AVR (video) → the TV, with the USB CEC adapter hanging off it as a CEC-only leg. Facts that fix the design: the AVR in standby powers down its CEC line and its NIC unless its menu enables network control in standby; the AVR's telnet port accepts one client; a TV cold-start needs WoL plus the TV's own wake-over-network setting; the adapter's tty has one owner and the loser fails silently.

**The adapter is not inline with video, and the box has only one HDMI out. Measured 2026-09-16.** The reference deployment exposes exactly **one** HDMI connector in DRM (`card1-HDMI-A-1`); its other eight outputs are DisplayPort and disconnected, so it cannot source two HDMI cables and never did. The Pulse-Eight adapter attaches to the HTPC by **USB only** (`/dev/ttyACM0`, `2548:1002`); its HDMI plug goes to an AVR input and carries **only the CEC line** — no video passes through it. Two consequences: the adapter's HDMI 2.0 ceiling does **not** constrain the 4K120 HDR path (that runs GPU → one HDMI 2.1 cable → AVR, measured at `3840x2160@119.98` with `GAMESCOPE_HDR_OUTPUT_FEEDBACK=1` before and after attaching), and the physical address the daemon claims is the *video* leg's, deliberately (see [`cec/README.md`](../cec/README.md) § "The physical address is explicit").

Until 2026-09-16 the adapter was **miswired as AVR → adapter IN, adapter OUT → AVR** — a loop, from which it derived a bogus physical address off the AVR's own EDID. **Open hypothesis:** some of the CEC flakiness §13 Q7 attributed to libcec may have been this topology rather than the library. Nothing here should assume the old failure modes will recur on the corrected wiring; they have to be re-observed before they are re-designed around.

**"A receiver ignores CEC from a non-selected input" is vendor-specific, and it does not bite on this AVR.** Measured 2026-09-16 from a non-selected input, with Moonlight the selected one: `<Give Audio Status>` → `<Report Audio Status>` vol 57 / mute off in **18 ms**; `<Give System Audio Mode Status>` → on in **39 ms**; a `<User Control Pressed>` volume-up plus its release moved the level **57 → 58**, confirmed by readback and then restored. So the Denon AVR-X1700H acts on CEC from an unselected input. The constraint is **narrowed, not deleted** — it is a real behaviour of other receivers, and the design rule it produced stands on its own: a transmit that "succeeded" still proves nothing, because a frame is ACKed by the hardware before anything acts on it. **Judge every action by readback, never by the transmit.** What the measurement does change is the claim, elsewhere in this section, that volume and standby *only* work while this box is the AVR's selected input: on this receiver they work either way, so claim-first ordering is prudence rather than a precondition.
...
|---|---|---|
| Wake / input select / standby of the AVR | `tv-shell-v2-cec`, **CEC primary** | `<Image View On>` + `<Active Source>` to claim, `<Set Stream Path>` to hand the display away, an **addressed** `<Standby>` to sleep. Ordering is claim-first, then act — prudence in this deployment rather than a precondition, since the AVR-X1700H was measured acting on CEC from a non-selected input (see above), but the guarantee is vendor-specific and the ordering costs nothing |
...
- **Only one supervisor may hold restart authority on the box.** The host-configuration Ansible role that provisions this deployment's TV box ships a **CEC watchdog** as a *system* unit that probes `cec-health`, reads a deliberately stopped `tv-shell-input` as a wedged adapter, and restarts it — re-grabbing the pad and silently undoing input fixes (jedwards1230/homelab-ansible#266). v2 introduces its own supervisor with restart authority, so at cutover the two would contend on one box.

  **The stand-down is DONE.** The role's CEC-watchdog toggle derives from its boot-session variable, the reference deployment's boot session is already `tv-shell-v2`, and the timer was read on the box on 2026-09-14 (read-only) as `disabled` / `inactive`. Nothing further is owed on the tv-shell side. Two things are still true and must stay said: the unit and timer **still exist on disk** under Ansible's ownership, so this is a disable rather than a removal and they must never be re-enabled; and the probe they run reads v1's `cec-health` IPC verb, which does not exist in v2 at all.
...
- **Fix the Ansible pin first** (jedwards1230/homelab-ansible#320): the role pins a pre-workspace-model daemon and downgrades the running one on any run. The v2 core and cec binaries now have a release stream — `session-v*`, one tag carrying both (§11) — so the pin becomes a version rather than a hand-typed SHA; the pin half is still to do. Packaging (#144, #147) remains the end state for install, upgrade and rollback.
...
- **New config** (`config.toml.example` rows and `daemon_config.rs` structs land with the core): `[display]` width/height/refresh, HDR default, SDR nits, hotplug settle; `[session]` shell app id, Xwayland count, the switch/map/launch-confirm bounds; `[supervisor]` stall seconds, restart thresholds, short-session window/count. Landed as `config/core.toml.example` + `core/src/config.rs` (its own file, not `config.toml` — v1's root is `deny_unknown_fields`). **The `[av]` half of that list never landed in `core.toml` and now never will**: §13 Q7 moved AV control out of the core into `tv-shell-v2-cec`, so the AVR host/port, input code, zone-2 policy and TV MAC/broadcast are `[avr]`/`[tv]` in `cec.toml` (a third config file, §11's rule applied once more), and the webOS host and key file are gone with the client that was never written (§8). The `[display]` mode and `session.xwayland_count` reach gamescope through an env file the session script renders with `tv-shell-core write-session-env` and the unit reads with a required `EnvironmentFile=`; a config key whose stated consumer does not exist is the repo's #416 class and a test asserts the link. **`[[app]]` and `[session].boot_app` landed 2026-09-06** — see below.
...
2. **Plugin mechanism** for Home Assistant and music streaming: manifest-driven `.desktop` extensions, a core-side registry, or out-of-process plugins over the IPC.
   ...
   **Deferred 2026-09-14**, deliberately and without picking a mechanism. A plugin needs a shell to plug into, and the shell currently has no install path at all: `/opt/tv-shell-v2/bin/` holds `tv-shell-core` and two gamescope scripts, `scripts/install-v2.sh` never touches `shell-v2/`, and there is no `tv-shell-v2-shell.service` for the session target to pull in. The target USED to name one anyway, as a forward reference; that was removed once the installer began refusing to install a target naming a unit it did not lay down, so the shell's absence is now visible rather than a silent `Wants=` skip. It joins the target in the same change that adds the unit file. Choosing between a `.desktop` extension, a core-side registry and out-of-process IPC plugins on that basis would be choosing an integration surface for a host that cannot be run, and the requirements paragraph below is deliberately all this question has. Revisit once there is a UI on the couch.
...
9. **Cause of the launch-coincident hotplug**: the AVR, an audio infoframe renegotiation at stream start, or the v1 CEC lifecycle. Discriminating runs: CEC off; Moonlight audio disabled. The relaunch-once policy waits on the answer.
   ...
   **The check is inconclusive**, and the reason is retention, not the hypothesis. journald on the reference deployment only reaches back to 2026-09-13 16:54 (43.8 MB on disk), so roughly **21 hours** could be surveyed rather than the eight days v2 has been running — and a survey of 21 hours that happened to contain no stream start proves nothing about eight days that did. To conclude, a stream start has to be **observed inside the current retention window**, or retention has to be increased so the window covers ordinary use.

   **Still open — but a launch was finally observed in-window, on 2026-09-16, and it did not hotplug.** A full Moonlight launch at 4K120 with `hdrMode=1` ran with CEC uninvolved: `GAMESCOPE_HDR_OUTPUT_FEEDBACK` held `1` across **507 samples at 10 Hz spanning the launch**, and the kernel logged **zero** DRM/HDMI/connector events in the window. That is evidence the launch does not *inherently* cause the hotplug — which matters for §6's relaunch-once fallback, whose worst case was "if the launch causes the hotplug it kills every first stream". It is **not** an exoneration of CEC: the event was not reproduced *at all*, so the run discriminates nothing between the three hypotheses. A negative result on a non-reproducing event is one sample, not an answer.

   Two limits on that sample, both worth carrying. **Coverage was 1 signal of 3**: the sampler watched only `GAMESCOPE_HDR_OUTPUT_FEEDBACK`; `GAMESCOPE_DISPLAY_SUPPORTS_HDR` and `GAMESCOPE_VRR_FEEDBACK` do not exist under those names on this build, so the other two legs of the hotplug signature were unwatched. And **attaching the adapter spends the free discriminator**: the reference deployment is now a CEC participant, so the zero-CEC experiment above is unavailable while the adapter is attached. It returns on reboot — the attach is non-persistent — and is gone **permanently** once `jedwards1230/homelab-ansible#339` merges and the kernel driver becomes part of the boot path. Any further zero-CEC survey has to be taken before that, or not at all.
