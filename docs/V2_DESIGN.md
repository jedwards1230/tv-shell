...
|---|---|---|
| Wake / input select / standby of the AVR | `tv-shell-v2-cec`, **CEC primary** | `<Image View On>` + `<Active Source>` to claim, `<Set Stream Path>` to hand the display away, an **addressed** `<Standby>` to sleep. Ordering is claim-first, then act — prudence on this rack rather than a precondition, since the AVR-X1700H was measured acting on CEC from a non-selected input (see above), but the guarantee is vendor-specific and the ordering costs nothing |
