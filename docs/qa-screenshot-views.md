...
| A4b | Widgets settings — list + drill-in | Settings ▸ Widgets | list of widget rows (Moonlight/Now Playing/Plex/Recent), each one focus stop: **A** toggles enable in place, **X**/**Right** drills into that widget's config sub-page (Size, Hide-from-Recent, and — Moonlight only — the full server-management surface inlined below Size). **B** steps back one level (sub-page → list → sidebar). |
