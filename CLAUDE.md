...
- **tv-shell-cec** (Rust binary, `cec/`) — the **v2** AV-control daemon, a NEW crate beside `core/` and never an evolution of `daemon/` (V2_DESIGN §13 Q12, same reason as the core: v1's `config.toml` root is `deny_unknown_fields`). It speaks the **kernel** CEC API (`/dev/cecN`, `pulse8-cec`) rather than libcec, which §13 Q7 made the primary AV control backend on 2026-09-14, demoting the IP leg to the cold path and to recovery. Third config file (`cec.toml`), third socket (`tv-shell-v2-cec.sock`), own unit (`core/units/tv-shell-v2-cec.service`) — installed by `install-v2.sh` (the fourth entry in its `UNITS=()`, and it builds this binary too) and `ConditionPathExists=/dev/cec0`-gated, so on a box with no adapter systemd records the unmet condition and starts nothing. It transmits **only when a client asks**: `wake`, `standby`, `input-claim`, `input-release`, `input-select <phys-addr>`, `volume up|down|mute|unmute` and `volume-state` are the whole transmit surface, beside the `ping` / `av-state` / `av-health` reads — nothing fires on a timer or a session event, and starting the daemon is still silent on the bus. Four rules are enforced by construction: a `<Standby>` is always **addressed, never broadcast** (a broadcast standby powers off every device on a shared bus); `standby` needs **positive proof** this box holds the display, with a refusal transmitting nothing at all and answering a distinct `refused:<why>` rather than `ok`; a `<User Control Pressed>` is **inseparable from its release** (one `Wire::KeyPair` value carries both, and the release goes out even on a NAKed press — an unreleased press auto-repeats on the AVR); and a volume action's success is judged from the AVR's own `<Report Audio Status>`, read before and after, **never from the transmit** — a receiver *may* ignore CEC from a non-selected input *after* ACKing the frame (vendor-specific — measured **not** to bite on a Denon AVR-X1700H on 2026-09-16, which acted on volume from an unselected input in 18-39 ms), so an unchanged level is an `error:` naming that cause. `mute`/`unmute` press CEC's `Mute` code, which is a **toggle**: an idempotent unmute is not expressible as one message on this bus, so the verbs read first and toggle only when the AVR is not already in the state asked for. The decision half is pure and lives beside the I/O (`ownership.rs` + `action.rs` + `volume.rs`), so CI covers every gate and the whole volume sequence with no adapter. Every `av-state` field is "what was observed and when" with `unknown` a first-class value that is never rendered as healthy or as `false` — a volume of `0` and an AVR that does not know its volume (CEC's `0x7F`) are different wire values. **`av-health` applies the same rule to health itself**: four independently observed facts (the fd answers `CEC_ADAP_G_CAPS`; the adapter holds an address; passive pin-level bus activity; last tx/rx as **ages**), a tri-state derived from the first two only, and `reason` naming which bus-liveness signal is in force — `CEC_CAP_MONITOR_PIN` is read, never assumed, and the daemon does not enter a pin-monitoring mode (it would replace follower mode and needs `CAP_NET_ADMIN`). Silence is published as a silence, never folded into the verdict. The `Type=notify` `WATCHDOG=1` feed is gated on the fd fact **alone** — not on the verdict, or every TV standby would be a restart loop — which makes systemd the box's only supervisor with restart authority here and is what **retires** the Ansible CEC watchdog rather than merely disabling it. **The IP leg is a capability complement first and a failover second.** Two things have no CEC expression at all — the receiver's **Zone 2** (`Z2OFF`) and a **cold wake of a television at mains standby** — so those steps (a Denon/Marantz ASCII telnet session, a Wake-on-LAN magic packet) run *before* the CEC steps of `wake`/`standby`, with a perfectly healthy bus; V2_DESIGN §13 Q7's "IP only when CEC is unavailable" was too narrow; the complement/failover distinction is now appended to Q7 and carried in §8. Standby is ordered that way for a second reason: a `Z2OFF` has to reach a receiver that is still awake. On top of that, `failover.rs` (PURE) decides the **warm** path and `backend` / `backend-pin cec|ip|auto` publish and override it. Its five rules: CEC is authoritative whenever the observed health is `Healthy`; **a single failed transmit does not fail over** (the trigger is N consecutive failures with no receive traffic in the same window, and `tx_error_threshold < 2` is refused at startup); hysteresis on **both** edges, from `cec.toml` and every threshold consumed (unlike `core.toml`'s `[supervisor]`); **un-failover is kernel-driven, not timer-driven** — the device stays open through a degraded period and a `PollResult::StateChange` re-reads the addressing, with a recovery committed only inside a fresh healthy health observation, never on elapsed time; and every change publishes the observation that caused it, as one log line and in `backend`'s reason. With no IP leg configured there is nowhere to fail over to, so `cec` stays active and the reason says so rather than announcing a backend that does not exist. The **television's IP leg is Wake-on-LAN only, write-only, with no state read** — there is no LG webOS/SSAP client in this tree and none should be written (a second auth surface with a history of breaking across firmware is one of the three reasons Q7 demoted IP). `[avr].main_power` is opt-in even on the IP path, because powering the main zone *down* can black out a television somebody is watching; an `ip`-carried `standby` without it answers `error:` naming the setting after sending the `Z2OFF` CEC could never send. All network I/O is behind the one-trait `ip::IpWire` seam, so **no test opens a socket or sends a packet**. This retires jedwards1230/tv-shell#251's *reboot* requirement — a wedge becomes a degraded mode recoverable in place — but **not** its USB-reset self-heal, which needs root or a udev-granted write and belongs with the privilege-model work (§13 Q8). The v2 session target `Wants=` this unit — a soft, unordered, non-fatal reference and the ONLY edge between it and the session, so a hung AV daemon is invisible to the compositor's supervision graph. It runs on the reference deployment, where the topology was measured on 2026-09-16 (the adapter negotiates onto the bus and the configured physical address reads back unchanged); what is still unobserved on hardware is a sustained degraded period and the recovery out of it. `tv-shell-cec --check-config` validates a staged `cec.toml` with the daemon's own parser before it is installed — the `validate:` shape `tv-shell-core write-session-env` already gives `core.toml` — and `config/cec.toml.example` is the annotated reference for the file. See [cec/README.md](cec/README.md).
- **tv-shell-v2** (Qt Quick app, `shell-v2/`) — the **v2** shell, built BESIDE v1's `shell/` and never instead of it, exactly as `core/` sits beside `daemon/`. Wired into no session. Four things it establishes and every later screen follows: an X11 **pre-map tagging shim** (`Surface` declares a *role* — base / overlay / toast — and the role drives the `STEAM_*` atoms, written before the window maps, since gamescope decides at window creation); **ONE QML module** (`TvShell`, via `qt_add_qml_module` — no hand-written `qmldir`, no relative-directory imports, against v1's eleven registries and 69 such imports); a **declarative focus graph** (a component declares *where it sits*, a router computes neighbours from the live cell set, judged once per event-loop turn, so disabling a widget or rebuilding a screen cannot strand focus); and **the core owns state** — the home screen is a pure function of `shell.json` plus one `screen-state` snapshot, reached over a single Unix socket (`CoreClient`), with **zero** `Process` sites against v1's 50. Two gaps this exposed in the core, reported rather than worked around: it publishes no event stream (so the shell requests a snapshot at the moments that can have changed it, and never polls) and no way to enumerate `[[app]]` (so the shell carries its own display-metadata catalog — [docs/V2_SHELL_CATALOG.md](docs/V2_SHELL_CATALOG.md)). Unlike v1's `shell/` it has a **CMake build** and is **not rsync-deployable** — see the Design Constraints note and [docs/V2_SHELL.md](docs/V2_SHELL.md). Decision logic stays in pure `.pragma library` JS, the one v1 pattern carried forward deliberately.
...
config/
  ...
  hyprland.conf.example       # A reference display example (LG C2/Denon HDR) → ~/.config/tv-shell/hyprland-local.conf
...
panel/                       # Rust web control panel (tv-shell-panel) — LAN-only, axum + askama + htmx
  src/                       # pages/ one module per route, grouped as the drawer is (docs/PANEL_IA.md):
                             ...
                             #   Shell appearance/widgets/apps/advanced · Devices controllers/display_audio/cec/av/network
...
cec/                         # Rust v2 AV control (tv-shell-cec) — kernel CEC, BESIDE core/
  src/
    config.rs                #   cec.toml — a THIRD file, separate from v1's and core.toml
    protocol.rs              #   IPC grammar, carried over from v1/core unchanged in contract
    ipc.rs                   #   Unix-socket server (0600); backend behind a trait so tests need no adapter
    backend.rs               #   The seam: linux-cec types stop here, never reach ipc.rs
    state.rs                 #   AvState + the rule that `unknown` is never healthy and never `false`
    ownership.rs             #   PURE: the tri-state ownership model + owns_display /
                             #   may_claim_active_source, ported from v1's display_owner.rs
    action.rs                #   PURE: a verb + two observed addresses -> a plan of messages,
                             #   or a Refusal that transmits NOTHING. Gates run before any build
    volume.rs                #   PURE: the volume/mute sequence — system-audio mode first, an
                             #   inseparable press/release pair, success judged from the read-back
    health.rs                #   PURE: four observed facts, a tri-state from two of them, and the
                             #   one rule the WATCHDOG=1 feed is gated on. `unknown` is never healthy
    notify.rs                #   sd_notify transport: READY=1 (Type=notify) and WATCHDOG=1
    failover.rs              #   PURE: which backend is authoritative on the WARM path. One NAK
                             #   never fails over; hysteresis on both edges; un-failover is a
                             #   kernel EVENT (StateChange), never a timer
    ip/
      mod.rs                 #   The IP leg: plan, judge, and the one-trait IpWire seam, so no
                             #   test opens a socket. Cold-path steps run BEFORE the CEC steps
      avr.rs                 #   Denon/Marantz ASCII telnet — ported from the never-merged #191
                             #   onto typed config. Z2OFF has no CEC equivalent: it goes on
                             #   EVERY standby
      wol.rs                 #   The magic packet, and nothing else. The TV's IP leg is
                             #   write-only: no webOS/SSAP client exists or should
    kernel/
      device.rs              #   /dev/cecN open + configure + read back. Order pinned: osd_name
                             #   BEFORE log addrs, or linux-cec transmits <Set OSD Name>
      follower.rs            #   The rx loop. Listens and folds; transmits nothing
      ops.rs                 #   PURE CecTx -> linux-cec Message table (no device, so CI covers
                             #   it), plus the transmitter + plan execution
shell-v2/                    # Qt Quick v2 shell (tv-shell-v2) — BESIDE shell/ (docs/V2_SHELL.md)
  ...
  src/
    ...
    shellconfig.{h,cpp}      #   Reads shell.json, hands QML its text. I/O only — no parsing
    paths.{h,cpp}            #   PURE: the core socket path and the catalog path
  qml/TvShell/               #   The single QML module — no qmldir, no relative-dir imports
    Main.qml                 #     Composition root: three SEPARATE toplevels + one core connection
    ...
    Card.qml                 #     The shell's ONE focusable type. One focus ring, everywhere
    Tokens.qml               #     Singleton: colour, type, space, duration. A literal elsewhere is a bug
  ...
  tests/                     #   5 lanes: surfacetags · coreclient · geometry · qml (offscreen) · premap (needs X)
...
- **Rust CI** (`rust.yml` / `host.yml`, reusable): `rust.yml` runs six jobs — the daemon (`tv-shell-input`) build/lint/test on Linux, a `--features cec` leg (static-libcec, on `rust:1-trixie`), a separate `panel` job that builds/lints/tests `tv-shell-panel` (pure-Rust, no system C deps), a `core` job that does the same for `tv-shell-core` (also pure-Rust — `x11rb` needs no libxcb/libX11), the `core-uinput` leg on the bare runner VM, and a `cec` job for `tv-shell-cec` (pure-Rust too — `linux-cec-sys` ships pre-generated bindings, so no bindgen, no libclang and no system libcec, which is why that job needs no apt step at all). `host.yml` builds/lints/tests the cross-platform host + protocol crates on Linux/macOS/Windows. All stay `-p`-scoped so no crate's graph leaks into another's build; `ci.yml`'s `rust` change filter covers `daemon/`, `panel/`, `protocol/`, `core/`, and `cec/`, so a QML-only PR triggers neither `rust.yml` nor `host.yml`.
...
- **Releases — four independent streams**: each artifact is versioned independently within the one monorepo, so the release trigger has to say *which* artifact to cut. The binary streams are **opt-in via a component-scoped label on the merged PR**; the widget stream stays tag-driven (a label can't carry the widget id).
  - `semver:host:patch|minor|major` → `release-host.yml` tags `host-v<semver>` and builds `tv-shell-host` for linux-musl / macOS (arm64+x86_64) / windows, publishing one Release with all binaries + `checksums.txt`. Consumed by the homelab desktop Ansible role in `jedwards1230/homelab-ansible`, which installs it with `install_method: fetch`.
  ...
  - `semver:session:patch|minor|major` → `release-v2.yml` tags `session-v<semver>` and builds the two v2 session binaries — `tv-shell-core` and `tv-shell-cec` — for `x86_64-unknown-linux-musl`, publishing **one** Release carrying both + `checksums.txt`. One tag for both because they are one session: `scripts/install-v2.sh` installs them together and the v2 session target names both units, so a core/cec version skew is a combination nothing tests. The binaries are static (no `PT_INTERP`), which is what the v2 crates' pure-Rust graph buys — `x11rb` with no libxcb, and `linux-cec-sys`'s pre-generated bindings with no libclang or system libcec. A fetch-by-tag install replaces cloning the repo at a hand-typed SHA and building on the deploy host, and rollback is selecting an older tag. `shell-v2/` is CMake/Qt, not a cargo crate, and is **not** built by this stream yet; the workflow's `V2_CRATES` list is the one place a further binary gets added.
  ...
  The three binary streams delegate version compute, tagging, and **AI release notes** to the shared `ai-release.yml@v1` reusable, each called with its own `tag_prefix` (`host-` / `input-` / `session-`) so it only ever reads and writes that artifact's tag series. **No label → no release**, so chore merges make no noise; a `semver:host:*` label never cuts a daemon or session release, and so on. All three also accept a manual `workflow_dispatch` (with `dry_run` to preview the version + notes). Do **not** hand-push a `host-v*` / `input-v*` / `session-v*` tag any more — the reusable owns those tags. The scoped labels are this repo's necessary variation on the org's flat `semver:*` convention; the widget stream remains a documented deviation.
  ...
  **The tag is the version.** Each binary workflow stamps the computed tag version into its own crate's `Cargo.toml` between the tag checkout and `cargo build`, then verifies the stamp landed — so `CARGO_PKG_VERSION` (`GET /status`, the MQTT state payloads, the HA `host_version`/`daemon_version` diagnostic entities, `build-info`, the MCP server version) is the real release version. It is stamped into the manifest rather than injected as a build env var precisely because cargo's fingerprint covers the package version, so a `rust-cache` hit can't serve an artifact carrying the previous one. The committed `version` in `host/`(`0.6.0`) and `daemon/`(`0.2.2`) tracks the last released tag as a floor for local/dev builds — bumping it releases nothing. `protocol` and `panel` sit at `0.0.0`: no tag series, and nothing reports their version.
  ...
  `core/` and `cec/` sit at `0.0.0` until the first `session-v*` tag, and they are the one case where **two manifests move together**: they ship as one session at one version, so a session release stamps BOTH to the same tag and its stamp-verify guard allows exactly those two manifests to move rather than one. They are also the only crates that report a revision **as well as** a version: each has a `build.rs` that stamps the git revision at **compile** time (`$TV_SHELL_BUILD_SHA` wins if set — the release workflow sets it to the commit it tagged — else `git rev-parse --short=12 HEAD` with `-dirty`, else `unknown`), and `tv-shell-core --version` / `tv-shell-cec --version` print `<binary> <version> (<sha>)` on stdout. Compile-time is the whole point: v1's `tv_shell_build_info` reads git at RUNTIME, so on a box with a source tree it reports that tree's HEAD rather than what the running binary was built from.

- **v2 shell** (`shell-v2.yml`, reusable): CMake-builds `shell-v2/`, runs the module-resolving `all_qmllint` target (whose DEFAULTS are weak, and no category can be promoted: qmllint does not resolve the C++ `Surface` type out of this static module on the pinned Qt 6.8, and with it unresolved a correct `Main.qml` reports as unqualified throughout — so that coverage comes from the `qml` lane running under `QT_FATAL_WARNINGS=1`, where an undefined binding fails the test; docs/V2_SHELL.md §11.9), and runs five ctest lanes — `surfacetags` (pure C++, no display), `coreclient` (the core IPC client against a real Unix socket), `geometry` (the ONLY lane that instantiates a real `Surface`: a Base window is its output's size, which is how a 160x160 default reached hardware), `qml` (the real `TvShell` module offscreen), and `premap` (a real `Xvfb`, asserting the `STEAM_*` `PropertyNotify` arrives BEFORE `MapNotify`). The `premap` lane is opt-in behind `TV_SHELL_TEST_XVFB` the way `core/`'s X tests are, and **this workflow sets it**; a dedicated step then asserts all five lanes were registered, because jedwards1230/tv-shell#469 was exactly a gated leg wired to nothing. `shell-v2/**` is its own `ci.yml` area — unlike v1's `qml` area it BUILDS, so it cannot ride inside `qml-test.yml`.
