...
shell-v2/                    # Qt Quick v2 shell (tv-shell-v2) — BESIDE shell/ (docs/V2_SHELL.md)
  ...
  tests/                     #   4 lanes: surfacetags · coreclient · qml (offscreen) · premap (needs X)
...
- **v2 shell** (`shell-v2.yml`, reusable): CMake-builds `shell-v2/`, runs the module-resolving `all_qmllint` target (whose DEFAULTS are weak, and no category can be promoted: qmllint does not resolve the C++ `Surface` type out of this static module on the pinned Qt 6.8, and with it unresolved a correct `Main.qml` reports as unqualified throughout — so that coverage comes from the `qml` lane running under `QT_FATAL_WARNINGS=1`, where an undefined binding fails the test; docs/V2_SHELL.md §11.9), and runs four ctest lanes — `surfacetags` (pure C++, no display), `coreclient` (the core IPC client against a real Unix socket), `qml` (the real `TvShell` module offscreen), and `premap` (a real `Xvfb`, asserting the `STEAM_*` `PropertyNotify` arrives BEFORE `MapNotify`). The `premap` lane is opt-in behind `TV_SHELL_TEST_XVFB` the way `core/`'s X tests are, and **this workflow sets it**; a dedicated step then asserts all four lanes were registered, because jedwards1230/tv-shell#469 was exactly a gated leg wired to nothing. `shell-v2/**` is its own `ci.yml` area — unlike v1's `qml` area it BUILDS, so it cannot ride inside `qml-test.yml`.
